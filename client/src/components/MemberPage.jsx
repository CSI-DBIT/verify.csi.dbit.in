...
const MemberPage = () => {
  ...
  const branch = ["COMP", "IT", "EXTC", "MECH"];
  const yr = ["Loading...", "FE", "SE", "TE", "BE"];
  const status = ["Inactive", "Active"];

  const [currentMembershipStatus, setCurrentMembershipStatus] =
    useState("Loading...");

  useEffect(() => {
    // Function to compare dates and set membership status
    const setMembershipStatus = () => {
      const currentDate = new Date();
      const startDate = new Date(studentDetails.startDate);
      const endDate = new Date(studentDetails.endDate);

      if (currentDate >= startDate && currentDate <= endDate) {
        setCurrentMembershipStatus("Active");
      } else {
        setCurrentMembershipStatus("Inactive");
      }
    };

    if (studentDetails) {
      setMembershipStatus(); // Call the function to set membership status
    }
  }, [studentDetails]);
    ...
    const fetchStudentCertificates = async () => {
      try {
        const response = await axios.get(
          `http://localhost:5000/api/certificate/${studentId}`
        );
    ...
    <div className="background">
            ...
            <div className="flip">
              <div className="front">
                ...
                <div className="investor">
                  {studentDetails ? studentDetails.role : "Loading..."}
                </div>
                <div className="chip">
                ...
                <div className="card-number">
                  <div className="section">
                    {studentDetails ? studentDetails.name : "Loading..."}
                  </div>
                ...
                <div className="card-holder">
                  {studentDetails ? studentDetails.studentId : "Loading..."}
                </div>
              ...
              <div className="back">
                ...
                <div className="terms">
                  <p>
                    ...
                    criminal offence. If found, please return to Monzo Bank or
      ...
      <div className="extra-card membership-details">
        ...
        <div className="datasection">
          <div className="section">
            Name: {studentDetails ? studentDetails.name : "Loading..."}
          </div>
          <div className="section">
            Membership Status:{" "}
            {studentDetails ? currentMembershipStatus : "Loading..."}
          </div>
          <div className="section">
            Start :{studentDetails ? studentDetails.startDate : "Loading..."}
          </div>
          <div className="section">
            End : {studentDetails ? studentDetails.endDate : "Loading..."}
          </div>
          ...
          <div className="section">
            Branch:{" "}
            {studentDetails ? branch[studentDetails.branch] : "Loading..."}
          </div>
          <div className="section">
            Year: {studentDetails ? yr[studentDetails.currentAY] : "Loading..."}
          </div>
        </div>
        <div className="section">
          {/* Certificates: 
            ...
            {studentCertificates
              ? studentCertificates.map((certificate, index) => {
                  return (
                    <>
                      <tr className="certi_table">
                        <td>{certificate.tag[0]}</td>
                        <td key={index} style={{ listStyle: "none" }}>
                          <Link
